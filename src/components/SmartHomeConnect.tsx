...
import {
  ...
  Play, Folder, Copy, Check, Plus, Trash2, ExternalLink, Home,
      ...
      <div className="flex-1 overflow-y-auto p-6 min-h-0 custom-scrollbar">
        ...
        {activeTab === 'google' && (
          <div className="max-w-3xl space-y-4">
            <div className="p-6 rounded-lg bg-[#0c0f18] border border-sky-500/20 space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-lg bg-sky-500/10 border border-sky-500/20 flex items-center justify-center text-sky-300">
                  <Home size={21} />
                </div>
                <div>
                  <h3 className="text-base font-bold text-white font-mono">Google Home</h3>
                  <p className="text-xs text-zinc-400">Controle sua casa por voz, sem depender do navegador.</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {['Tuya configurada', 'Conta OSONE conectada', 'Ponte segura na nuvem'].map((label, index) => (
                  <div key={label} className="px-3 py-2.5 rounded-lg bg-black/35 border border-white/5 flex items-center gap-2">
                    <CheckCircle2 size={13} className={index === 0 && estado !== 'pronto' ? 'text-zinc-600' : 'text-emerald-400'} />
              ...
              {onOpenSettings && (
                <button
                  onClick={onOpenSettings}
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-sky-500 hover:bg-sky-400 text-black font-bold text-xs font-mono transition-colors"
                >
                  <Settings size={14} />
                  <span>Ativar no Google Home</span>
                </button>
              )}

              <p className="text-[11px] text-zinc-500 leading-relaxed">
                A ativação fica em Ajustes &gt; Automação. O instalável prepara a conta e mostra um código temporário;
                não é necessário publicar servidor, abrir porta ou criar projeto no console do Google.
              </p>
            </div>
          </div>
        )}
