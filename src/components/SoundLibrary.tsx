...
import { 
  ...
  Image, Video, Camera, Film, UserPlus, ChevronRight, Download, ExternalLink, ShieldAlert, Copy, Check,
  Info, Globe, Sliders, Tag, Radio, Key, AlertTriangle, RefreshCw
} from 'lucide-react';
...
export interface FreesoundItem {
  ...
  previewUrl: string;
  tags: string[];
  category: 'music' | 'effect' | 'ambient' | 'other';
  freesoundUrl: string;
}
...
export const SoundLibrary = ({ 
  ...
  const [currentTab, setCurrentTab] = useState<'freesound' | 'favorites' | 'sounds' | 'music_playlists' | 'gallery'>('freesound');

  // Freesound Search States
  const [freesoundQuery, setFreesoundQuery] = useState('chuva');
  const [freesoundCategory, setFreesoundCategory] = useState<'all' | 'music' | 'effect' | 'ambient'>('all');
  const [freesoundLicense, setFreesoundLicense] = useState<'cc0' | 'all'>('cc0');
  const [freesoundApiKeyInput, setFreesoundApiKeyInput] = useState('');
  const [freesoundResults, setFreesoundResults] = useState<FreesoundItem[]>([]);
  ...
  const [freesoundNeedsKey, setFreesoundNeedsKey] = useState(false);

  // Favorites States
  const [favorites, setFavorites] = useState<FreesoundItem[]>([]);
  const [favoriteSearch, setFavoriteSearch] = useState('');
  const [isLoadingFavorites, setIsLoadingFavorites] = useState(false);

  // License Modal State
  const [attributionModalItem, setAttributionModalItem] = useState<FreesoundItem | null>(null);
  const [copiedCredit, setCopiedCredit] = useState(false);

  // Freesound Search Action
  const handleSearchFreesound = async (customQuery?: string) => {
    const q = customQuery !== undefined ? customQuery : freesoundQuery;
    setIsSearchingFreesound(true);
    setFreesoundError(null);
    setFreesoundNeedsKey(false);
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        if (data.needsApiKey) {
          setFreesoundNeedsKey(true);
        } else {
          setFreesoundError(data.error || 'Erro ao conectar à API do Freesound.');
        }
        setFreesoundResults([]);
      } else {
        setFreesoundResults(data.items || []);
        if (data.needsApiKey) {
          setFreesoundNeedsKey(true);
        }
      }
    } catch (err: any) {
      setFreesoundError(err.message || 'Erro ao carregar áudios do Freesound.');
      setFreesoundResults([]);
    } finally {
  ...
  useEffect(() => {
    fetchFavorites();
    handleSearchFreesound('chuva');
  }, []);

  const handleToggleFavorite = async (item: FreesoundItem, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    const isFav = favorites.some(f => String(f.id) === String(item.id));
    try {
      if (isFav) {
        const res = await fetch(`/api/library/favorites/${item.id}`, { method: 'DELETE' });
        if (res.ok) {
        ...
        if (res.ok) {
          setFavorites(prev => [item, ...prev.filter(f => String(f.id) !== String(item.id))]);
        }
      }
    } catch (err) {
  ...
  const handlePlayFreesoundItem = (item: FreesoundItem) => {
    if (playingUrl === item.previewUrl) {
      onStopSound();
    } else {
      onPlaySound(item.previewUrl);
    }
  };

  const triggerDownloadFile = (url: string, filename: string) => {
  ...
  const handleDownloadClick = (item: FreesoundItem) => {
    if (item.requiresAttribution) {
      setAttributionModalItem(item);
      setCopiedCredit(false);
    } else {
      triggerDownloadFile(item.previewUrl, `${item.name.replace(/[^a-zA-Z0-9_-]/g, '_')}.mp3`);
    }
  };

  const handleCopyCreditAndDownload = (item: FreesoundItem) => {
    navigator.clipboard.writeText(item.attributionText);
    setCopiedCredit(true);
    triggerDownloadFile(item.previewUrl, `${item.name.replace(/[^a-zA-Z0-9_-]/g, '_')}.mp3`);
    setTimeout(() => {
      setAttributionModalItem(null);
      setCopiedCredit(false);
    }, 1800);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-her-bg/50 backdrop-blur-xl overflow-hidden w-full relative">
      ...
      {/* Navigation Sub-Tabs */}
      <div className="flex px-4 md:px-8 border-b border-white/[0.05] bg-black/10 shrink-0 gap-3 md:gap-6 overflow-x-auto custom-scrollbar">
        <button 
          onClick={() => setCurrentTab('freesound')}
          className={cn(
            "py-4 text-[10px] uppercase font-semibold tracking-[0.15em] border-b-2 transition-all flex items-center gap-2 shrink-0 cursor-pointer",
            currentTab === 'freesound' 
      ...
      {/* TAB FREESOUND ONLINE */}
      {currentTab === 'freesound' && (
        <div className="flex-1 flex flex-col min-h-0 p-4 md:p-8 gap-5 overflow-y-auto custom-scrollbar">
          {/* Header Bar & Search */}
          <div className="flex flex-col gap-4 p-5 rounded-2xl bg-[#161514] border border-white/[0.05]">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
              <div>
                <h3 className="text-sm font-bold text-amber-100 flex items-center gap-2">
                  ...
                  Biblioteca Freesound (Sons, Músicas e Ambientes Gratuitos)
                </h3>
                <p className="text-[11px] text-zinc-400 mt-0.5">
                  Pesquise na biblioteca global do Freesound.org e baixe em alta qualidade para seus vídeos.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 bg-black/40 p-1 rounded-xl border border-white/[0.05]">
                <button
                  onClick={() => { setFreesoundLicense('cc0'); handleSearchFreesound(); }}
                  className={cn(
                  ...
                  title="Apenas Domínio Público (Sem necessidade de créditos)"
                >
                  <ShieldAlert size={12} className="text-emerald-400" />
                  <span>Domínio Público (CC0)</span>
                </button>

                <button
                  onClick={() => { setFreesoundLicense('all'); handleSearchFreesound(); }}
                  className={cn(
                    "px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5",
                    freesoundLicense === 'all'
                      ? "bg-amber-500/20 text-amber-300 border border-amber-500/40"
                      : "text-zinc-400 hover:text-white"
                  )}
                  title="Inclui licenças que exigem atribuição/créditos ao autor"
                >
                  <Info size={12} className="text-amber-400" />
                  <span>Todas (Inclui Atribuição)</span>
                </button>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Buscar por palavra-chave (ex: chuva, aplausos, risadas, lofi, piano, mar, vento)..."
                  value={freesoundQuery}
                  onChange={(e) => setFreesoundQuery(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && handleSearchFreesound()}
                  className="w-full bg-[#1e1d1b] border border-white/[0.08] py-3 pl-10 pr-4 text-xs text-white rounded-xl focus:outline-none focus:border-amber-500/50"
                />
              </div>
              ...
              <button
                ...
                className="px-5 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 hover:brightness-110 text-white text-xs font-bold uppercase tracking-wider flex items-center gap-2 cursor-pointer shadow-lg shadow-amber-500/20 transition-all"
              >
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-white/[0.04]">
              <span className="text-[11px] text-zinc-500 font-medium mr-1">Categoria:</span>
              {[
                { id: 'all', label: 'Todos', icon: Disc },
                { id: 'effect', label: 'Efeitos Sonoros', icon: Volume2 },
                { id: 'music', label: 'Músicas', icon: Music },
                { id: 'ambient', label: 'Ambiente', icon: Radio },
              ].map(cat => {
                const IconComp = cat.icon;
                const isActive = freesoundCategory === cat.id;
                return (
                  <button
                    key={cat.id}
                    onClick={() => { setFreesoundCategory(cat.id as any); handleSearchFreesound(); }}
                    className={cn(
                      "px-3 py-1.5 rounded-xl text-xs font-medium transition-all flex items-center gap-1.5 cursor-pointer border",
                      isActive
                        ? "bg-amber-500/20 text-amber-300 border-amber-500/40 font-semibold"
                        : "bg-white/[0.02] text-zinc-400 hover:text-white border-white/[0.05]"
                    )}
                  >
                    <IconComp size={13} />
                    <span>{cat.label}</span>
            ...
            <div className="flex items-center gap-1.5 text-[10px] text-zinc-500 flex-wrap">
              <span>Sugestões:</span>
              {['chuva', 'aplausos', 'passos', 'trilha lo-fi', 'fogo', 'vento', 'teclado', 'risadas'].map(sug => (
                <button
                  key={sug}
                  onClick={() => { setFreesoundQuery(sug); handleSearchFreesound(sug); }}
                  className="px-2 py-0.5 rounded-lg bg-white/[0.03] hover:bg-white/[0.08] text-zinc-300 transition-colors cursor-pointer"
                >
                  {sug}
                </button>
              ))}
          ...
          {/* API Key Alert Panel if needed */}
          {freesoundNeedsKey && (
            <div className="p-4 rounded-2xl bg-amber-950/40 border border-amber-500/30 text-xs text-amber-200 flex flex-col gap-3">
              <div className="flex items-start gap-2.5">
                <Key size={18} className="text-amber-400 shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-bold text-amber-300">Cole sua Chave do Freesound (FREESOUND_API_KEY)</h4>
                  <p className="text-[11px] text-zinc-300 mt-1 leading-relaxed">
                    Você pode gerar uma chave grátis em{' '}
                    <a
                    ...
                    e colá-la no seu arquivo <code className="bg-black/50 px-1 py-0.5 rounded text-amber-300">.env</code> ou direto no campo abaixo:
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <input
                  type="text"
                  placeholder="Cole sua FREESOUND_API_KEY aqui..."
                  value={freesoundApiKeyInput}
                  onChange={(e) => setFreesoundApiKeyInput(e.target.value)}
                  className="flex-1 bg-black/60 border border-amber-500/30 rounded-xl px-3 py-2 text-xs font-mono text-white focus:outline-none"
                />
                <button
                  onClick={() => handleSearchFreesound()}
          ...
          {/* Results List */}
          {isSearchingFreesound ? (
            <div className="flex-1 min-h-[300px] flex flex-col items-center justify-center text-center p-8 border border-dashed border-white/[0.05] rounded-2xl">
              <Sparkles size={32} className="text-amber-400 animate-spin mb-3" />
              <p className="text-xs text-zinc-300 font-medium">Buscando áudios na biblioteca do Freesound...</p>
              <p className="text-[10px] text-zinc-500 mt-1">Filtrando por licença e categorias em tempo real</p>
            </div>
          ) : freesoundError ? (
            <div className="p-6 rounded-2xl bg-rose-950/30 border border-rose-500/30 text-rose-200 text-xs text-center flex flex-col items-center gap-2">
              <AlertTriangle size={24} className="text-rose-400" />
              <p className="font-semibold">{freesoundError}</p>
              <button
                onClick={() => handleSearchFreesound()}
                className="mt-2 px-4 py-1.5 rounded-xl bg-rose-500/20 text-rose-300 border border-rose-500/30 text-[11px] hover:bg-rose-500/30 transition-colors cursor-pointer"
              >
                Tentar Novamente
              </button>
            </div>
          ) : freesoundResults.length === 0 ? (
            <div className="flex-1 min-h-[250px] flex flex-col items-center justify-center text-center p-8 border border-dashed border-white/[0.05] rounded-2xl">
              <Music size={32} className="text-zinc-600 mb-2" />
              <p className="text-xs text-zinc-400">Nenhum som encontrado para os filtros selecionados.</p>
              <p className="text-[10px] text-zinc-600 mt-1">Tente pesquisar por outros termos como "chuva", "lofi", "applause" ou alterar o filtro de licença.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {freesoundResults.map((item) => {
                const isPlaying = playingUrl === item.previewUrl;
                const isFav = favorites.some(f => String(f.id) === String(item.id));
                ...
                return (
                  <div
                    key={item.id}
                    className={cn(
                      "group p-3.5 rounded-2xl bg-[#141312] border transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3",
                      isPlaying
                        ? "border-amber-500/60 bg-amber-950/10 shadow-lg shadow-amber-500/10"
                        : "border-white/[0.05] hover:border-white/[0.12] hover:bg-[#181716]"
                    )}
                  >
                    {/* Left: Play Button + Sound Info */}
                    <div className="flex items-center gap-3.5 flex-1 min-w-0">
                      <button
                        onClick={() => handlePlayFreesoundItem(item)}
                        className={cn(
                          "w-11 h-11 rounded-2xl flex items-center justify-center transition-all shrink-0 cursor-pointer shadow-md",
                          isPlaying
                            ? "bg-amber-500 text-black shadow-amber-500/30 scale-105 animate-pulse"
                            : "bg-white/[0.05] text-white group-hover:bg-amber-500/20 group-hover:text-amber-300"
                        )}
                        title={isPlaying ? "Pausar som" : "Ouvir prévia em áudio"}
                      >
                      ...
                      <div className="flex flex-col min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h4 className="text-xs font-bold text-white truncate max-w-[300px]">{item.name}</h4>
                          
                          {/* License Badge */}
                          {item.requiresAttribution ? (
                            <span className="text-[9px] font-mono px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-300 border border-amber-500/30 flex items-center gap-1" title="Exige citação de créditos">
                              <Info size={10} /> CC-BY Atribuição
                            </span>
                          ) : (
                            <span className="text-[9px] font-mono px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 flex items-center gap-1" title="Domínio público livre de créditos">
                              <ShieldAlert size={10} /> CC0 Domínio Público
                            </span>
                          )}
                        </div>

                        <div className="flex items-center gap-3 text-[10px] text-zinc-400 mt-1 font-mono">
                          <span>Autor: <strong className="text-zinc-300">@{item.username}</strong></span>
                          <span>•</span>
                          <span>Duração: <strong className="text-amber-400">{item.formattedDuration}</strong></span>
                        </div>
                        ...
                        {item.tags.length > 0 && (
                          <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                            {item.tags.slice(0, 4).map(tag => (
                              <span key={tag} className="text-[9px] text-zinc-500 bg-black/40 px-2 py-0.5 rounded-md border border-white/[0.03]">
                                #{tag}
                              </span>
                            ))}
                          </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                      <button
                        ...
                        className={cn(
                          ...
                          isFav
                            ...
                            : "bg-white/[0.02] text-zinc-400 hover:text-rose-400 border-white/[0.05]"
                        )}
                      ...
                      <button
                        ...
                        className="px-3.5 py-2 rounded-xl bg-amber-500/15 hover:bg-amber-500 text-amber-300 hover:text-black border border-amber-500/30 font-mono text-[11px] font-bold flex items-center gap-1.5 transition-all cursor-pointer"
                      >
                        <Download size={14} />
                        <span>Baixar</span>
                      </button>
      ...
      {currentTab === 'favorites' && (
        <div className="flex-1 flex flex-col min-h-0 p-4 md:p-8 gap-5 overflow-y-auto custom-scrollbar">
          <div className="flex items-center justify-between pb-3 border-b border-white/[0.05]">
            <div>
              <h3 className="text-sm font-bold text-rose-200 flex items-center gap-2">
                ...
                Sons Favoritos Salvos ({favorites.length})
              </h3>
              <p className="text-[11px] text-zinc-400 mt-0.5">
                Seus áudios favoritados persistidos no arquivo favorites.json.
              </p>
            </div>
          ...
          {favorites.length === 0 ? (
            <div className="flex-1 min-h-[250px] flex flex-col items-center justify-center text-center p-8 border border-dashed border-white/[0.05] rounded-2xl">
              <Heart size={32} className="text-zinc-600 mb-2" />
              <p className="text-xs text-zinc-400">Você ainda não tem áudios favoritados.</p>
              <p className="text-[10px] text-zinc-600 mt-1">Acesse a aba Freesound e clique no coração para salvar seus efeitos e músicas favoritos!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {favorites.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={cn(
                      "p-3.5 rounded-2xl bg-[#141312] border transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3",
                      isPlaying
                        ? "border-rose-500/60 bg-rose-950/10 shadow-lg shadow-rose-500/10"
                        : "border-white/[0.05] hover:border-white/[0.12] hover:bg-[#181716]"
                    )}
                  >
                    <div className="flex items-center gap-3.5 flex-1 min-w-0">
                      <button
                        ...
                        className={cn(
                          ...
                          isPlaying
                            ...
                            : "bg-white/[0.05] text-white hover:bg-rose-500/20 hover:text-rose-300"
                        )}
                      >
                        {isPlaying ? <Pause size={18} /> : <Play size={18} className="ml-0.5" />}
                      </button>
                      ...
                      <div className="flex flex-col min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-3 text-[10px] text-zinc-400 mt-1 font-mono">
                          <span>Autor: <strong className="text-zinc-300">@{item.username}</strong></span>
                          <span>•</span>
                          <span>Duração: <strong className="text-rose-400">{item.formattedDuration}</strong></span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                      <button
                        onClick={() => handleDownloadClick(item)}
                        className="px-3.5 py-2 rounded-xl bg-white/[0.05] hover:bg-amber-500 text-zinc-300 hover:text-black border border-white/[0.08] font-mono text-[11px] font-bold flex items-center gap-1.5 transition-all cursor-pointer"
                      >
                        <Download size={14} />
                      ...
                      <button
                        onClick={(e) => handleToggleFavorite(item, e)}
                        className="p-2.5 rounded-xl bg-rose-500/20 text-rose-300 hover:bg-rose-500/40 border border-rose-500/40 transition-all cursor-pointer"
      ...
      <AnimatePresence>
        {attributionModalItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-[#181716] border border-amber-500/40 rounded-3xl p-6 max-w-lg w-full shadow-2xl flex flex-col gap-4 text-white"
            >
              <div className="flex items-center justify-between border-b border-white/[0.06] pb-3">
                <div className="flex items-center gap-2.5 text-amber-400">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-amber-200">Aviso de Licença e Atribuição</h3>
                    <p className="text-[10px] text-zinc-400 font-mono">Licença: {attributionModalItem.license}</p>
                  </div>
                </div>
                <button
                  onClick={() => setAttributionModalItem(null)}
                  className="p-1.5 text-zinc-400 hover:text-white rounded-xl hover:bg-white/[0.05] cursor-pointer"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <p className="text-xs text-zinc-300 leading-relaxed">
                Este áudio é gratuito, mas exige a citação dos créditos abaixo na descrição do seu vídeo ou publicação.
              </p>
              ...
              <div className="p-3.5 rounded-2xl bg-black/60 border border-white/[0.08] flex flex-col gap-2">
                <div className="flex items-center justify-between text-[10px] font-mono text-zinc-400">
                  <span>Texto de Créditos para Copiar:</span>
                  {copiedCredit && <span className="text-emerald-400 font-bold flex items-center gap-1"><Check size={12} /> Copiado!</span>}
                </div>
                <textarea
                  readOnly
                  value={attributionModalItem.attributionText}
                  className="w-full bg-transparent border-none text-xs font-mono text-amber-300 focus:outline-none resize-none h-16"
                />
              </div>

              <div className="flex flex-col gap-2 mt-1">
                <button
                  onClick={() => handleCopyCreditAndDownload(attributionModalItem)}
                  className="w-full py-3 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 hover:brightness-110 text-white text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-lg shadow-amber-500/20 cursor-pointer"
                >
                  <Copy size={15} />
                  <span>{copiedCredit ? 'Crédito Copiado! Baixando MP3...' : 'Copiar Crédito & Baixar MP3'}</span>
                </button>

                <button
                  onClick={() => {
                    triggerDownloadFile(attributionModalItem.previewUrl, `${attributionModalItem.name.replace(/[^a-zA-Z0-9_-]/g, '_')}.mp3`);
                    setAttributionModalItem(null);
                  }}
                  className="w-full py-2.5 rounded-2xl bg-white/[0.03] hover:bg-white/[0.08] text-zinc-400 hover:text-white text-[11px] font-mono transition-colors cursor-pointer"
                >
                  Apenas Baixar sem Copiar
                </button>
              </div>
            </motion.div>
          </div>
